...
df = df[-30*24*period:].reset_index(drop=True)
...
for s in range(len(seeds)):
    for ele in range(len(elements)):
        for iter in range(iteration):
            ...
            # For missing rate change
            # name = case + '_result_'+ str(seeds[s])+'_DNN_'+str(elements_name[ele])+'_missing rate_'+str(int(missing_ratio*10))+'_'+str(iter+1)

            # For period change
            name = case + '_result_' + str(seeds[s]) + '_DNN_' + str(elements_name[ele]) + '_period_' + str(int(period)) + '_' + str(iter + 1)
            eraser = df.sample(int(len(df)*missing_ratio), random_state=seeds[s]).index
            target = elements[ele]
